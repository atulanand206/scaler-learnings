package com.atul.scaler.lean.strings.substringworepeats;
...
public class Solution {
  ...
  public int lengthOfLongestSubstring(String A) {
    ...
    int max = i;
    while (i < A.length()) {
      while (i < A.length() && !set.contains(A.charAt(i))) set.add(A.charAt(i++));
      max = Math.max(max, i - s);
      while (s + 1 < i && i < A.length() && A.charAt(s) != A.charAt(i)) set.remove(A.charAt(s++));
      i++;

      s++;
      debug(string(A.substring(s - 1, i - 1)));
      max = Math.max(max, i - s);
    }
    return max;
  }
